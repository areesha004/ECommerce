  ...
  <img onClick={()=> token? null:navigate('/Login')} src={assets.profile_icon} className='w-4 cursor-pointer' alt="" />
{
  ...
  <div className=' group-hover:block absolute hidden dropdown-menu right-0 pt-4'>
      <div className='flex flex-col w-36 gap-2 py-3 px-5 bg-slate-100 text-gray-500 rounded'>
        ...
        <p onClick={logoutBtn} className='cursor-pointer hover:text-black'>Logout</p>
      </div>
 </div>
}
...
<Link to={'./cart'} className='relative'>
 ...
 <p className='absolute text-center right-[-5px] bottom-[-5px] w-4 leading-4 bg-black text-white aspect-square rounded-full text:sm'>{getCartCount()}</p>
 </Link>
...
<div className={`absolute top-0 right-0 bottom-0 overflow-hidden bg-white transition-all text-gray-600 ${visible?" w-full" :" w-0"}`}>
